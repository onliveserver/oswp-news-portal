...
function getDraftKey(id) {
  return `oswp_portal_edit_post_draft_${id}`;
}
...
export default function EditPostPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      postFields.forEach((f) => {
        if (f.type !== 'tab' && f.type !== 'media') {
          init[f.id] = post[f.id] ?? post.meta?.[f.id] ?? '';
        }
      });
      const storedDraft = readDraft(getDraftKey(id));
      setForm(storedDraft?.form ? { ...init, ...storedDraft.form } : init);
      if (post.thumbnail_url) setPreview(post.thumbnail_url);
      setLoading(false);
    }).catch(() => navigate('/posts'));
  }, [id]);

  useEffect(() => {
    if (loading || !id || !Object.keys(form).length) {
      return;
    }

    writeDraft(getDraftKey(id), { form });
  }, [form, loading, id]);
  ...
  const getValidationErrors = useCallback((err) => {
    ...
    return { submit: err?.message || 'Failed to update post.' };
  }, []);

  const titleFieldId = fields.find((f) => ['post_title', 'title', 'post_name'].includes(f.id))?.id || '';  const tagsFieldId  = fields.find((f) => ['post_tags', 'tags', 'post_tag'].includes(f.id))?.id || '';
  ...
  const handleSubmit = async (e) => {
    ...
    fields.forEach((field) => {
      ...
      // Validate tags using admin-managed post settings
    ...
    try {
      ...
      await toastPromise(() => postsApi.update(id, fd), {
        loading: 'Saving post…',
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ marginTop: 16, display: 'flex', gap: 10 }}>
                  <button type="submit" className="oswp-btn oswp-btn-primary" style={{ width: 'auto' }} disabled={busy}>
                    {busy ? (
                      <><span className="oswp-btn-spinner" style={{ borderColor: 'rgba(255,255,255,0.4)', borderTopColor: '#fff' }} />Saving…</>
                    ) : 'Save changes'}
                  </button>
